...
def new_fast_params(named_parameters,grads,update_rate):
    def get_grad(g):
        if g is None:
            return 0
        return g
    named_parameters = [ (n,p-update_rate*get_grad(g)) for (n,p), g in zip(named_parameters,grads) ]
...
class MultiConcreteHeadAttention(nn.Module):

    NEW_ID = itertools.count()

    def __init__(self, n_heads, dim, dropout,temp=2/3,epsilon=0.1):
        super().__init__()
        self.layer_id = next(MultiConcreteHeadAttention.NEW_ID)
        self.dim = dim
        self.n_heads = n_heads
        self.dropout = dropout
        assert self.dim % self.n_heads == 0
        ...
        self.v_lin = Linear(dim, dim)
        self.out_lin = Linear(dim, dim)
        self.concrete_gate = ConcreteGate(n_heads, temp, epsilon)
...
class MetaTransformerModel(TransformerModel):
    ...
    def predict(self, tensor, pred_mask, y, get_scores,params=None):
        ...
        """
        masked_tensor = tensor[pred_mask.unsqueeze(-1).expand_as(tensor)].view(-1, self.dim)
        scores, loss = self.pred_layer(masked_tensor, y, get_scores, params)
        return scores, loss