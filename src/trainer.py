...
class Trainer(object):
    ...
    def __init__(self, data, params):
        ...
        if params.multi_gpu and params.amp == -1:
            ...
            for name in self.MODEL_NAMES:
                import inspect
                if 'find_unused_parameters' in inspect.getargspec(nn.parallel.DistributedDataParallel):
                    setattr(self, name,
...
class MultiDomainTrainer(Trainer):
    ...
    def get_iterator(self, iter_name, lang1, lang2, domain):
        ...
        _lang1, _lang2 = (lang1, lang2) if lang1 < lang2 else (lang2, lang1)
        iterator = self.data['para'][(_lang1, _lang2, domain)]['train'].get_iterator(
            shuffle=True,
            group_by_size=self.params.group_by_size,
            n_sentences=-1,
        )
...
class CurriculumTrainer(Trainer):
    ...
    def get_iterator(self, iter_name, lang1, lang2):
        ...
        batches, _ = dataset.get_iterator(
            shuffle=False,
            group_by_size=self.params.group_by_size,
            n_sentences=-1,
        )

        batches = self.order_curriculum(batches,dataset)

        iterator = dataset.get_batches_iterator(batches,False)
...
class DualEncoderTrainer(Trainer):
    ...
    def mt_step(self, lang1, lang2, lambda_coeff):
        ...
        enc2 = enc2.transpose(0, 1)
        enc1_list = [ e[:l].mean(dim=0) for e, l in zip(enc1,len1)]
        enc2_list = [ e[:l].mean(dim=0) for e, l in zip(enc2,len2)]
        sims = []
        for e1 in enc1_list:
            sim = []
            for e2 in enc2_list:
        ...
        emb1_loss += -torch.log(torch.diagonal(numerator) / numerator.sum(dim=1)).sum()
        ...
        emb2_loss = 0
        # for batch in torch.split(numerator, int(len(numerator) / self.split_size)):  # batch : (split_size,split_size)
        emb2_loss += -torch.log(torch.diagonal(numerator) / numerator.sum(dim=1)).sum()
